...
| Mailchimp API Key | GL `mailchimp-api-key`: 32 case-insensitive hex characters then -us and two decimal digits, under Mailchimp context (the rule starts with `(?i)`). Ours restricts hex to lowercase and allows one or two digits without context. | Keep distinctive data-centre suffix with one-digit compatibility extension. No generic 32-hex promotion. |
